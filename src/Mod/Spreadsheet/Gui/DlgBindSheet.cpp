...
DlgBindSheet::DlgBindSheet(Sheet *sheet, Range range, QWidget *parent)
    : QDialog(parent), sheet(sheet), range(range), ui(new Ui::DlgBindSheet)
{
    ui->setupUi(this);
    ...
    ui->lineEditToStart->setDocumentObject(sheet);
    ui->lineEditToStart->setPrefix('=');
    ui->lineEditToEnd->setDocumentObject(sheet);
