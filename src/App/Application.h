...
#define FC_APP_SIGNAL(_sig,_name) FC_SIGNAL_NOEXCEPT(App,_sig,_name)

    /** @name Signals of the Application */
    //@{
    /// signal on new Document
    FC_APP_SIGNAL(void (const Document&, bool),signalNewDocument);
    /// signal on document getting deleted
    FC_APP_SIGNAL(void (const Document&),signalDeleteDocument);
    /// signal on already deleted Document
    FC_APP_SIGNAL(void (),signalDeletedDocument);
    /// signal on relabeling Document (user name)
    FC_APP_SIGNAL(void (const Document&),signalRelabelDocument);
    /// signal on renaming Document (internal name)
    FC_APP_SIGNAL(void (const Document&),signalRenameDocument);
    /// signal on activating Document
    FC_APP_SIGNAL(void (const Document&),signalActiveDocument);
    /// signal on saving Document
    FC_APP_SIGNAL(void (const Document&),signalSaveDocument);
    /// signal on starting to restore Document
    FC_APP_SIGNAL(void (const Document&),signalStartRestoreDocument);
    /// signal on restoring Document
    FC_APP_SIGNAL(void (const Document&),signalFinishRestoreDocument);
    /// signal on starting to save Document
    FC_APP_SIGNAL(void (const Document&, const std::string&),signalStartSaveDocument);
    /// signal on saved Document
    FC_APP_SIGNAL(void (const Document&, const std::string&),signalFinishSaveDocument);
    /// signal on undo in document
    FC_APP_SIGNAL(void (const Document&), signalUndoDocument);
    /// signal on application wide undo
    FC_APP_SIGNAL(void (), signalUndo);
    /// signal on redo in document
    FC_APP_SIGNAL(void (const Document&), signalRedoDocument);
    /// signal on application wide redo
    FC_APP_SIGNAL(void (), signalRedo);
    /// signal before close/abort active transaction
    FC_APP_SIGNAL(void (bool), signalBeforeCloseTransaction);
    /// signal after close/abort active transaction
    FC_APP_SIGNAL(void (bool), signalCloseTransaction);
    /// signal on show hidden items
    FC_APP_SIGNAL(void (const Document&),signalShowHidden);
    /// signal on start opening document(s)
    FC_APP_SIGNAL(void (),signalStartOpenDocument);
    /// signal on finished opening document(s)
    FC_APP_SIGNAL(void (),signalFinishOpenDocument);
    //@}
    ...
    FC_APP_SIGNAL(void (const App::Document&, const App::Property&),signalBeforeChangeDocument);
    /// signal on changed doc property
    FC_APP_SIGNAL(void (const App::Document&, const App::Property&),signalChangedDocument);
    /// signal on new Object
    FC_APP_SIGNAL(void (const App::DocumentObject&),signalNewObject);
    //FC_APP_SIGNAL(void (const App::DocumentObject&),    m_sig);
    /// signal on deleted Object
    FC_APP_SIGNAL(void (const App::DocumentObject&),signalDeletedObject);
    /// signal on changed Object
    FC_APP_SIGNAL(void (const App::DocumentObject&, const App::Property&),signalBeforeChangeObject);
    /// signal on changed Object
    FC_APP_SIGNAL(void (const App::DocumentObject&, const App::Property&),signalChangedObject);
    /// signal on relabeled Object
    FC_APP_SIGNAL(void (const App::DocumentObject&),signalRelabelObject);
    /// signal on activated Object
    FC_APP_SIGNAL(void (const App::DocumentObject&),signalActivatedObject);
    /// signal before recomputed document
    FC_APP_SIGNAL(void (const App::Document&),signalBeforeRecomputeDocument);
    /// signal on recomputed document
    FC_APP_SIGNAL(void (const App::Document&),signalRecomputed);
    /// signal on recomputed document object
    FC_APP_SIGNAL(void (const App::DocumentObject&),signalObjectRecomputed);
    // signal on opened transaction
    FC_APP_SIGNAL(void (const App::Document&, std::string),signalOpenTransaction);
    // signal a committed transaction
    FC_APP_SIGNAL(void (const App::Document&),signalCommitTransaction);
    // signal an aborted transaction
    FC_APP_SIGNAL(void (const App::Document&),signalAbortTransaction);
    //@}
    ...
    FC_APP_SIGNAL(void (const App::Property&),signalAppendDynamicProperty);
    /// signal on about removing a dynamic property
    FC_APP_SIGNAL(void (const App::Property&),signalRemoveDynamicProperty);
    /// signal on about changing the editor mode of a property
    FC_APP_SIGNAL(void (const App::Document&, const App::Property&),signalChangePropertyEditor);
    //@}
