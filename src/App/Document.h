...
#define FC_DOC_SIGNAL(_sig, _name)  FC_SIGNAL_NOEXCEPT(App::Document, _sig, _name)

    /** @name Signals of the document */
    //@{
    /// signal before changing an doc property
    FC_DOC_SIGNAL(void (const App::Document&, const App::Property&), signalBeforeChange);
    /// signal on changed doc property
    FC_DOC_SIGNAL(void (const App::Document&, const App::Property&), signalChanged);
    /// signal on new Object
    FC_DOC_SIGNAL(void (const App::DocumentObject&), signalNewObject);
    //boost::signals2::signal<void (const App::DocumentObject&)>     m_sig;
    /// signal on deleted Object
    FC_DOC_SIGNAL(void (const App::DocumentObject&), signalDeletedObject);
    ...
    FC_DOC_SIGNAL(void (const App::DocumentObject&), signalTouchedObject);
    /// signal on relabeled Object
    FC_DOC_SIGNAL(void (const App::DocumentObject&), signalRelabelObject);
    /// signal on activated Object
    FC_DOC_SIGNAL(void (const App::DocumentObject&), signalActivatedObject);
    /// signal on created object
    FC_DOC_SIGNAL(void (const App::DocumentObject&, Transaction*), signalTransactionAppend);
    /// signal on removed object
    FC_DOC_SIGNAL(void (const App::DocumentObject&, Transaction*), signalTransactionRemove);
    /// signal on undo
    FC_DOC_SIGNAL(void (const App::Document&), signalUndo);
    /// signal on redo
    FC_DOC_SIGNAL(void (const App::Document&), signalRedo);
    ...
    FC_DOC_SIGNAL(void (const App::Document&, const std::vector<App::DocumentObject*>&), signalRecomputed);
    FC_DOC_SIGNAL(void (const App::DocumentObject&), signalRecomputedObject);
    //signal a new opened transaction
    FC_DOC_SIGNAL(void (const App::Document&, std::string), signalOpenTransaction);
    // signal a committed transaction
    FC_DOC_SIGNAL(void (const App::Document&), signalCommitTransaction);
    // signal an aborted transaction
    FC_DOC_SIGNAL(void (const App::Document&), signalAbortTransaction);
    FC_DOC_SIGNAL(void (const App::Document&, const std::vector<App::DocumentObject*>&), signalSkipRecompute);
    FC_DOC_SIGNAL(void (const App::DocumentObject&), signalFinishRestoreObject);
    FC_DOC_SIGNAL(void (const App::Document&,const App::Property&), signalChangePropertyEditor);
    //@}
