...
public:
     ...
     * @sa traceElement()
     */
    typedef std::function<bool(const MappedName &, int, long)> TraceCallback;
	 ...
	 *            cb(const MappedName &name, int offset, long tag),
     *            where 'name' is the current element name, 
     *            'offset' is the offset skipping the encoded element name for the next iteration,
     *            'tag' is the current geometry tag.
	 *            The callback can return 'true' to terminate the iteration.
     */
