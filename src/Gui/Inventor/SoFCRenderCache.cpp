  ...
  if (parent.texturematrices.getNum()) {
    for (auto & v : parent.texturematrices.getData()) {
      ...
      if (info.identity)
        ...
        info.matrix = info.matrix.multLeft(v.second.matrix);
      info.identity = false;
    ...
    for (const auto & info : child.lights.getData()) {
      if (info.resetmatrix)
        ...
        if (copy.identity)
          ...
          copy.matrix = copy.matrix.multLeft(matrix);
        copy.identity = false;
                                ...
                                bool opencache)
{
  ...
  bool identity = (elem->getModelMatrix() == matrixidentity);
  ...
  this->caches.emplace_back(elem->getModelMatrix(),
                            identity,
                            this->resetmatrix,
                            cache,
                            vcache);
...
SoFCRenderCache::getVertexCaches(bool finalize)
{
  ...
  for (auto & entry : PRIVATE(this)->caches) {
    ...
    for (auto & child : entry.cache->getVertexCaches()) {
      ...
      for (const VertexCacheEntry & childentry : child.second) {
        ...
        if (entry.identity || childentry.resetmatrix)
          ...
          it->second.emplace_back(childentry.cache,
                                  entry.matrix.multRight(childentry.matrix),
                                  false,
                                  false,
                                  key);
      }
