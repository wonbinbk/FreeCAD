...
public:
  ...
  static std::size_t pushDrawEntry(std::vector<DrawEntry> & entries,
                                   const Material & material,
                                   const VertexCacheEntry & ventry);
...
inline std::size_t
SoFCRendererP::pushDrawEntry(std::vector<DrawEntry> & entries,
                             const Material & material, 
                             const VertexCacheEntry & ventry)
{
  entries.emplace_back(&material, &ventry);
  if (entries.back().bbox.isEmpty()) {
    entries.pop_back();
    return 0;
  }
  return entries.size();
...
SoFCRenderer::setScene(const std::vector<RenderCachePtr> & caches)
{
  ...
  for (auto & cache : PRIVATE(this)->scene) {
    for (auto & entry : cache->getVertexCaches(true)) {
      if (entry.second.empty()) continue;
      if (entry.first.drawstyle == SoDrawStyleElement::INVISIBLE) continue;

      bool fulltransp = entry.first.transptexture;
      if (!fulltransp && !entry.first.pervertexcolor)
        fulltransp = (entry.first.diffuse & 0xff) == 0xff ? false : true;

      for (auto & ventry : entry.second) {
        std::size_t idx = SoFCRendererP::pushDrawEntry(PRIVATE(this)->drawentries, entry.first, ventry);
        if (!idx)
          continue;
        --idx;
...
SoFCRenderer::setHighlight(VertexCacheMap && caches)
{
  ...
  for (auto & entry : PRIVATE(this)->highlightcaches) {
    if (entry.second.empty()) continue;
    if (entry.first.drawstyle == SoDrawStyleElement::INVISIBLE) continue;
    ...
    for (auto & ventry : entry.second) {
      std::size_t idx = SoFCRendererP::pushDrawEntry(PRIVATE(this)->hlentries, entry.first, ventry);
      if (!idx)
        continue;
      --idx;
...
SoFCRendererP::updateSelection()
{
  ...
  for (auto & sel : this->selectionsontop) {
    for (auto & entry : *sel.second) {
      if (entry.second.empty()) continue;
      if (entry.first.drawstyle == SoDrawStyleElement::INVISIBLE) continue;

      for (auto & ventry : entry.second) {
        std::size_t idx = checkKey(entry.first, ventry);
        if (!idx)
          continue;
        --idx;
...
SoFCRendererP::setupMatrix(SoState * state, const VertexCacheEntry * ventry)
{
  if (this->identity) {
    if (ventry->identity)
      ...
      SoModelMatrixElement::set(state, NULL, this->matrix.multLeft(ventry->matrix));
}
                            ...
                            int pass)
{
  ...
  for (std::size_t idx : indices) {
    ...
    if (entry.skip)
      continue;
    if (this->recheckmaterial || this->prevmaterial != entry.material) {
      if (!applyMaterial(action, *entry.material, false, pass))
        continue;
      this->recheckmaterial = false;
      this->prevmaterial = entry.material;
    }
    setupMatrix(state, entry.ventry);
    ...
    case Material::Triangle:
      if (!entry.ventry->cache->hasTransparency())
        entry.ventry->cache->renderTriangles(state, array, entry.ventry->partidx);
      else if (!this->material.pervertexcolor) {
        // this means override transparency (i.e. force opaque)
        entry.ventry->cache->renderTriangles(state, SoFCVertexCache::NON_SORTED, entry.ventry->partidx);
      }
      ...
      break;
    case Material::Line:
      entry.ventry->cache->renderLines(state, array, entry.ventry->partidx);
      break;
    case Material::Point:
      entry.ventry->cache->renderPoints(state, array, entry.ventry->partidx);
      break;
    }
  }
}
                                  ...
                                  bool sort)
{
  ...
  if (sort) {
    ...
    if (plane != this->prevplane) {
      ...
      for (auto & v : indices)
        v.distance = plane.getDistance(entries[v.idx].bbox.getCenter());
  ...
  for (auto & v : indices) {
    auto & entry = entries[v.idx];
    if (entry.skip)
      continue;
    if (this->recheckmaterial || this->prevmaterial != entry.material) {
      if (!applyMaterial(action, *entry.material, true))
        continue;
      this->recheckmaterial = false;
      this->prevmaterial = entry.material;
    }
