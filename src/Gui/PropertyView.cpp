        ...
        if(obj && vpLast->getLinkedView(true)!=vpLast) {
            ...
            for(auto v : props) {
                ...
                if(obj->isHidden(prop) || prop->testStatus(App::Property::Hidden))
                    continue;
                std::vector<App::Property*> value(1,prop);
                dataProps.push_back(std::make_pair(v.first, value));
                linkDataProps.insert(v.first);
            }
            props.clear();
            vpLast->getPropertyMap(props);
            for(auto v : props) {
                auto prop = v.second;
                if(vpLast->isHidden(prop) || prop->testStatus(App::Property::Hidden))
                    continue;
                std::vector<App::Property*> value(1,prop);
                viewProps.push_back(std::make_pair(v.first, value));
                linkViewProps.insert(v.first);
            }
        }
    }
