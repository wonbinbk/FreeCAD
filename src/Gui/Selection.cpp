...
void SelectionSingleton::setVisible(int visible) const {
    std::set<std::pair<ViewProvider*,ViewProvider*> > filter;
    if(visible<0) 
        visible = -1;
    else if(visible>0)
        visible = 1;
    for(auto &sel : _SelList) {
        ...
        ViewProvider *parentVp;
        if(parent && (parentVp=Application::Instance->getViewProvider(parent)) && 
           parentVp->hasChildElement()) 
        {
            // prevent setting the same object visibility more than once
            if(!filter.insert(std::make_pair(vp,parentVp)).second)
                continue;

            const char *sub = obj->getNameInDocument();
            int vis = parentVp->isElementVisible(sub);
            if(vis>=0) {
                if(vis>0) vis = 1;
                if(visible>=0) {
                    if(vis == visible)
                ...
                parentVp->setElementVisible(sub,vis?true:false);
                continue;
            }
        ...
        if(!filter.insert(std::make_pair(vp,(ViewProvider*)0)).second)
            continue;
